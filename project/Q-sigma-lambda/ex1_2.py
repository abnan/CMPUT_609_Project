...
write_data = ','.join(str(x) for x in lambdas)
#write header
...
f.write("Exponential decay sigma"+',')
f.write(write_data+'\n')
#Plot all different lr for a fixed sigma
plt.plot(lambdas, avg_reward_list, colours[colour_index], label = "Exponential decay sigma")
colour_index += 1
