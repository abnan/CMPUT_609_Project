    ...
    for run in range(num_run):
        ...
        for epi_num in range(episodes):
            ...
            for epi_len in itertools.count():
                ...
                if(epi_num>0):
                    sigma=clamp(abs(td_error/td_avg), 0, 1)
                ...
                if done:
                    ...
                    td_avg = np.mean(td_error_list_this_episode)
...
f.write("TD error sigma; lambda = 0.7"+',')
f.write(write_data+'\n')
#Plot all different lr for a fixed sigma
plt.plot(alphas, avg_reward_list, colours[colour_index], label = "TD error sigma; lambda = 0.7")
colour_index += 1
