...
for lmbda in lambda_list:
    for lr in alpha_list:
        for n_run in range(num_run):
            # print(n_run)
            episode_steps, returns, weights, _ = qSigmaMC(env, max_size=10000, n_episodes = num_episodes, sigma=0.95, Lambda=lmbda, alpha=lr, render=False, printing=False)

        avg_rewards = []
        std_error = []
        for i in range(30, num_episodes):
            avg_rewards.append(np.mean(returns_dict[i]))
            std_error.append(np.std(returns_dict[i])/np.sqrt(num_run))
        print("Lambda = ", lmbda, "LR = ", lr, "Average reward = ",np.mean(avg_rewards), np.max(avg_rewards))
        area = np.trapz(avg_rewards)
        print("area =", area)
# rc_moving_avg_rewards = []
# for i in range(50, len(avg_rewards)):
#     rc_moving_avg_rewards.append(np.mean(avg_rewards[i-50:i]))
