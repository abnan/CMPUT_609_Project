...
class StochasticWindyGridworldEnv(discrete.DiscreteEnv):
    ...
    def step(self, action):
        ...
        if(self.epi_num==1):
            self.end_point = tuple([3,7])

        
        if (tuple(s_n) == self.end_point):
            self.done = True
            self.epi_num+=1
            self.new_dict=dict()
            if(self.epi_num%self.change_duration==0):
                x = np.random.choice(np.arange(4))
                y = np.random.choice(np.arange(10))
                self.end_point = tuple([x, y])
                # print(self.epi_num, self.end_point)
