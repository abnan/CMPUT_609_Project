...
for lmbda in lambda_list:
    for lr in alpha_list:
        for n_run in range(num_run):
            # print(n_run)
            episode_steps, returns, weights, _ = qSigmaMC(env, max_size=10000, n_episodes = num_episodes, sigma=1, Lambda=lmbda, alpha=lr, render=False, printing=False)
        ...
        print("Lambda = ", lmbda, "LR = ", lr, "Average reward = ",np.mean(avg_rewards))
