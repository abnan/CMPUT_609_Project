...
for n_run in range(num_run):
    print(n_run)
    episode_steps, returns, weights, _ = qSigmaMC(env, n_episodes = num_episodes, sigma=1, Lambda=0.1, alpha=0.5, render=False, printing=False)
...
plt.plot(np.arange(len(avg_rewards[50:])), avg_rewards[50:], label = "Max TD error sigma; lambda=0.5")
plt.xlabel('episodes')
