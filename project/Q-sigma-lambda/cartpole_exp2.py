...
def qSigmaMC(env,  n_episodes = 100, Lambda = 0, sigma = 1, epsilon = 0.1, alpha = 0.1, gamma = 1, 
    ...
    for i in range(n_episodes):
        ...
        while done == False:
            ...
            sigma_list_this_episode.append(sigma)
            ...
            if done:
                ...
                sigma_average.append(np.mean(sigma_list_this_episode))
                if printing:
...
for n_run in range(num_run):
    print(n_run)
    episode_steps, returns, weights, _ = qSigmaMC(env, max_size=10000, n_episodes = num_episodes, sigma=1, Lambda=0.7, alpha=0.5, render=False, printing=False)
