...
for alph in alphas:
    for lmbd in lmbds:
        ...
        for n_run in range(num_run):
            # print(n_run)
            episode_steps, returns, weights, _ = qSigmaMC(env, n_episodes = num_episodes, sigma=0.95, Lambda=lmbd, alpha=alph, render=False, printing=False)

        avg_rewards = []
        std_error = []
        for i in range(50, num_episodes):
            avg_rewards.append(np.mean(returns_dict[i]))
            std_error.append(np.std(returns_dict[i])/np.sqrt(num_run))
        print("Average for this run=", np.mean(avg_rewards))
        area = np.trapz(avg_rewards)
        print("Area =", area)
        rc_moving_avg_rewards = []
        for i in range(30, len(avg_rewards)):
            rc_moving_avg_rewards.append(np.mean(avg_rewards[i-30:i]))
        # print(np.average(rc_moving_avg_rewards[-100:]), np.std(rc_moving_avg_rewards[-100:]))
        plt.plot(np.arange(len(rc_moving_avg_rewards)), rc_moving_avg_rewards, label = "Max TD error sigma; lambda=0.5")
        # plt.plot(np.arange(len(avg_rewards)), avg_rewards, label = "Max TD error sigma; lambda=0.5")
        plt.xlabel('episodes')
